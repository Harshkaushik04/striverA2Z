...
int find_index(vector<int>& arr,int i,int j,int to_find){
    for(int k=i;k<=j;k++){
        if(arr[k]==to_find) return k;
    }
    return -1;
}

void build_tree(int preorder_i,int preorder_j,vector<int>& preorder,
    int inorder_i,int inorder_j,vector<int>& inorder,TreeNode* godfather,bool direction){
    ...
    int index=find_index(inorder,inorder_i,inorder_j,current);
    if(direction==0) {
        godfather->left=new TreeNode(preorder[preorder_i]);
        child=godfather->left;
    }
...
TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
    ...
    build_tree(0,preorder_size-1,preorder,0,preorder_size-1,inorder,godfather,0); //direction==0 for left, direction==1 for right
